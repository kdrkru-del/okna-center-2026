...
export default function PricingTable({ hideTitle = false }: { hideTitle?: boolean }) {
  ...
  return (
    <section id="prices" className={`bg-white text-slate-900 ${hideTitle ? "py-4" : "py-20 border-t border-slate-200/80"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {!hideTitle && (
          <div className="text-center max-w-3xl mx-auto mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 mb-4">
              Популярные окна и цены
            </h2>
            <p className="text-slate-600 text-sm sm:text-base font-light">
              Честные цены напрямую от производителя во Владивостоке. Без скрытых наценок. Итоговая смета фиксируется в договоре и не меняется.
            </p>
          </div>
        )}
