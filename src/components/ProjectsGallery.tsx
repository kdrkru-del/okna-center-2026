...
export default function ProjectsGallery({ limit }: { limit?: number }) {
  ...
  return (
    <section id="projects" className="py-20 bg-[#060B12] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <Link
            ...
            className="px-5 py-2.5 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-full text-xs font-mono uppercase tracking-wider transition-colors inline-flex items-center gap-2 self-start md:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {displayItems.map((item) => (
            <div
              ...
              className="group relative rounded-2xl overflow-hidden bg-slate-900 border border-white/10 cursor-pointer aspect-4/3 hover:border-cyan-400/50 transition-all hover:shadow-[0_10px_30px_rgba(6,182,212,0.15)]"
            >
              <Image
                src={asset(item.src)}
                alt={item.title}
        ...
        {limit && galleryItems.length > limit && (
          <div className="text-center mt-12">
            <Link
              ...
              className="px-8 py-3.5 bg-white/10 hover:bg-white/20 text-white rounded-2xl text-xs uppercase font-mono tracking-wider transition-colors inline-block"
            >
